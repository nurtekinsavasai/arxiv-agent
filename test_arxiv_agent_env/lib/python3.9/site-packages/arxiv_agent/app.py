...
#   pip install streamlit requests feedparser openai
...
# Minimum number of papers we want in the citation prediction set (when possible)
...
def fetch_arxiv_papers_by_date(
    ...
    end_date: date,
    batch_size: int = 50,
    max_batches: int = 100,   # 100 * 50 ≈ 5000 papers
    max_retries: int = 3,
) -> List[Paper]:
    """
    Fetch cs.AI + cs.LG papers from arXiv between start_date and end_date (inclusive),
    up to roughly batch_size * max_batches papers.
    """
...
def call_llm(prompt: str, llm_config: LLMConfig, label: str = "") -> str:
    ...
    try:
        ...
        # Reasoning models like o1 do not support temperature
...
def safe_parse_json_array(raw: str) -> Optional[List[Dict[str, Any]]]:
    """
    Try to parse an LLM response as a JSON array, being tolerant of:
    - Leading/trailing text
    - Markdown code fences
    - Extra commentary before/after the array
    """
    ...
    # Try to locate the JSON array between first '[' and last ']'
    start = text.find("[")
    end = text.rfind("]")
...
def embed_texts(
    ...
    """
    Compute embeddings for a list of texts, batching requests to stay within
    OpenAI's max tokens per request limit.
    """
    if not texts:
        return []
    ...
    # Conservative batch size so that title+abstract texts stay well under token limits.
    batch_size = 100
...
# =========================
# Direct citation prediction helpers
# =========================
...
# =========================
# Plain English summary helper
# =========================

def summarize_paper_plain_english(paper: Paper, llm_config: LLMConfig) -> str:
    """
    Summarize a paper in plain English for a smart non-technical reader,
    using only the title and abstract.
    """
...
def main():
    ...
    # Top row: short tagline + demo video button
...
The agent ranks papers by predicted citation count, always showing **primary** papers first, then secondary ones. For the top N that you choose, it shows a plain English summary, relevance explanation, and links to arXiv and the PDF. All intermediate artifacts and a markdown report are saved in a project folder under `~/arxiv_ai_digest_projects/project_<timestamp>`, and you can download everything as a ZIP.
    ...
    with st.sidebar:
        ...
        # Reassurance note under API key
        st.caption(
        ...
        st.caption(f"Embedding model (fixed): `{embedding_model_name}`")

        run_clicked = st.button("🚀 Run Pipeline")

    # Track sidebar parameters to know when they change
    params = {
        "research_brief": research_brief.strip(),
        "not_looking_for": not_looking_for.strip(),
    ...
    # If sidebar params changed and Run was not clicked, clear cached results and instruct user to run
    if params != st.session_state["last_params"] and not run_clicked:
        # Clear cached pipeline outputs
        for key in [
            "current_papers",
            "candidates",
            "used_papers",
    ...
    # Update last_params when Run is clicked (so subsequent changes get detected)
    if run_clicked:
        st.session_state["last_params"] = params.copy()

    # Validate API key/model only when we need to run or reuse results
    if not api_key or not model_name:
        # If we already have cached results, we can still show them without re-running
        if "ranked_papers" not in st.session_state:
            st.warning("Your OpenAI API key and chat model name are required to run the pipeline.")
            return
    ...
    # Store mode and query_brief so we can reuse them on reruns
    ...
    # If user has never run the pipeline and didn't click Run, show instructions and exit
    ...
    # 1. Project setup (always show, but only create a new folder when running)
    ...
    if mode == "global":
        ...
        st.success(f"{len(candidates)} top candidates selected by embedding similarity for LLM classification.")
    ...
    # 4. LLM classification (or skip in global mode)
    st.subheader("4. LLM Relevance Classification")

    if mode == "global":
        st.info(
            "Global mode: no specific research brief was provided. "
            "Skipping LLM relevance classification and treating all candidate papers as PRIMARY."
        )
        # Only set focus labels when running the pipeline freshly
        ...
        # We re-run classification only when Run is clicked or we don't have labels yet
        if run_clicked or any(p.focus_label is None for p in candidates):
            with st.spinner("Classifying candidates as PRIMARY, SECONDARY, or OFF TOPIC..."):
    ...
    if mode == "global":
        ...
        st.success(
            f"Global mode: using {len(used_papers)} most recent cs.AI + cs.LG papers for citation prediction."
        )
    else:
    ...
    st.write(
        ...
        "Selection is automatic based on mode, embeddings (in targeted modes), and LLM relevance classification."
    )
...
The predicted citation counts are heuristic impact signals, not precise forecasts. They are best used for ranking and prioritization within this batch of papers, not as ground truth. The model may reflect existing academic biases, for example favoring well known authors, large labs, English language venues, or hot topics. These scores should never be used on their own for hiring, promotion, funding, or other formal evaluation decisions.
    ...
    if run_clicked or "ranked_papers" not in st.session_state:
        ...
        if not papers_with_pred:
            st.error("LLM did not return predictions for any selected papers.")
            return
    ...
    # 8. Top N highlighted using PRIMARY-first ranked list
    ...
    # Cache plain-English summaries per paper so they are not recomputed on download button clicks
    ...
    report_lines = [
        ...
        "Provider: OpenAI",
        f"OpenAI Chat model: {model_name}",
    ...
    # 10. ZIP download – build only on run, reuse bytes on button clicks
